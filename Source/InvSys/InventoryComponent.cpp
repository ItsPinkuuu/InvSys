...
int32 UInventoryComponent::HandleStackableItems(UItemBase* ItemIn, int32 RequestedAddAmount)
{
	...
	while (ExistingItemStack)
	{
		...
		else if (WeightLimitAddAmount <= 0)
		{
			...
			{
				// this block will be reached if distributing on item across multiple stacks
				// and the weight limit is hit during the process
				OnInventoryUpdated.Broadcast();
				return RequestedAddAmount - AmountToDistribute;
			}

			// reached if there is a partial stack but none if it can be added at all
			return 0;
		}
...
void UInventoryComponent::AddNewItem(UItemBase* Item, const int32 AmountToAdd)
{
	...
	{
		// Used when slitting or dragging to/from another inventory
